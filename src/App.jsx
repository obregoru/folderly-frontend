...
export default function App() {
  ...
  // with the same per-clip settings. The new entry is appended after
  // the source so the user sees it immediately. Source must be
  // persisted (have a _dbFileId); otherwise there's nothing to copy on
  // the server. While the copy is in flight we mark the source's
  // _duplicating flag so the button shows a spinner.
  const duplicateFile = useCallback(async (sourceItem) => {
    ...
    try {
      ...
      // Insert right after the source so the duplicate sits next to its
      // origin. If the source is gone for some reason, append to the end.
      setFiles(prev => {
