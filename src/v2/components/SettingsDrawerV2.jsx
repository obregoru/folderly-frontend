    ...
    <div className="fixed inset-0 z-30 flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-3">
        <div>
          ...
          <SectionCard
            icon="🤖" label="AI activity log"
            open={expanded === 'ailog'}
            onToggle={() => toggle('ailog')}
            desc="Every AI call — prompt in, response out, tokens, duration. Useful for debugging or comparing against other models."
          >
            <AiLogView open={expanded === 'ailog'} />
          </SectionCard>
...
function AiLogView({ open }) {
  ...
  const load = async () => {
    ...
    try {
      const r = await api.getAiLog({ limit: 100, endpoint: endpointFilter || undefined })
      if (Array.isArray(r)) setRows(r)
      else if (r?.error) throw new Error(r.error)
    } catch (e) { setErr(e.message || String(e)) }
    finally { setLoading(false) }
  }
  useEffect(() => { if (open) load() /* eslint-disable-next-line */ }, [open, endpointFilter])

  const clearLog = async () => {
    if (!confirm('Clear the entire AI log for this tenant? This cannot be undone.')) return
    try { await api.clearAiLog(); setRows([]); setExpandedId(null) }
    catch (e) { setErr(e.message || String(e)) }
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1.5 flex-wrap">
        <button onClick={load} disabled={loading} className="text-[10px] py-1 px-2 border border-[#e5e5e5] rounded bg-white cursor-pointer disabled:opacity-50">
          {loading ? 'Loading…' : 'Refresh'}
        </button>
        <select value={endpointFilter} onChange={e => setEndpointFilter(e.target.value)} className={`${inp} w-auto flex-1`}>
          <option value="">All endpoints</option>
          {endpoints.map(e => <option key={e} value={e}>{e}</option>)}
        </select>
        <button onClick={clearLog} className="text-[10px] py-1 px-2 border border-[#c0392b]/40 text-[#c0392b] rounded bg-white cursor-pointer">Clear all</button>
      </div>

      {err && <div className="text-[10px] text-[#c0392b]">{err}</div>}

      {!loading && rows.length === 0 && (
        <div className="text-[10px] text-muted italic py-3 text-center">
          No AI interactions logged yet. Generate captions, best times, or auto-name a draft to see entries here.
        </div>
      )}
