  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        {draftId && (
          <AudioMixLog draftId={draftId} segments={segments} primaryAudioUrl={audioUrl} primaryDuration={primaryDuration} />
        )}
      ...
      {/* Live captioned preview used to mount here as a separate
          fold with its own @remotion/player video element. Retired:
          captions now paint directly on the main merged-video
          element at the top of the editor via InlineCaptionOverlay
          inside FinalPreviewV2. No duplicate <video> decoder, no
          separate Player runtime — the video already on screen IS
          the preview. Download still hits /post/render-final for
          the authoritative full-res mp4 unchanged. */}
...
// AudioMixLog — a transparent diagnostic of what render-final will
// actually mix into the exported video. Mirrors the BE logic at
// social-post.js: primary plays from job.voiceover_audio_key when no
// in-session primary_audio_base64 is sent, plus every segment with an
// audioKey at its startTime. The synthetic __primary__ segment is
// EXCLUDED from segment mix on the BE (audioKey:null + explicit reject
// of the reserved id), so we exclude it here too — the row labeled
// "Primary" comes from voiceover_audio_key, not from the segment.
function AudioMixLog({ draftId, segments, primaryAudioUrl, primaryDuration }) {
  ...
  const tracks = useMemo(() => {
    if (!job) return []
    const rows = []
    // Primary: voiceover_audio_key if present (matches the BE's else-if
    // fallback when primary_audio_base64 isn't sent). The Download
    // button DOES send primary_audio_base64, but that input is ephemeral
    // and not stored — the persisted one is what every other render
    // path uses, so it's the truth source for "what's in the export".
    if (job.voiceover_audio_key || job.voiceover_audio_url) {
      ...
      rows.push({
        ...
        number: 1,
        startTime: 0,
        // Duration: prefer the primary segment's persisted duration
        // → in-session primaryDuration → unknown.
        duration: Number(primarySeg?.duration) || Number(primaryDuration) || null,
    ...
    const ordered = segs
      ...
      .sort((a, b) => (Number(a.startTime) || 0) - (Number(b.startTime) || 0))
    ordered.forEach((s, i) => {
      const willMix = !!s.audioKey  // BE filter
      rows.push({
        kind: 'segment',
        number: i + 2,
        id: s.id,
        ...
        text: s.text || '',
        audioKey: s.audioKey || null,
        warn: willMix ? null : 'no audio — will NOT mix',
      })
    })
    return rows
  }, [job, primaryDuration])
  ...
  const shortKey = (k) => {
    if (!k) return '—'
    return k.length > 32 ? `…${k.slice(-32)}` : k
  }
  ...
  return (
    <>
      <button
        ...
        className="text-[10px] py-1 px-2.5 border border-[#6C5CE7] text-[#6C5CE7] bg-white rounded cursor-pointer"
        title="Show every audio track that render-final will mix into the exported video, in time order"
      >🔍 Audio mix log</button>
      {open && (
        <div
        ...
        >
          <div
            ...
            className="bg-white rounded-lg shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-auto p-4 space-y-3"
          >
            <div className="flex items-center gap-2">
              <div className="text-[14px] font-medium flex-1">🔍 Audio mix log</div>
              <button
                onClick={() => setOpen(false)}
                className="text-[12px] py-1 px-2 border border-[#e5e5e5] rounded bg-white cursor-pointer"
              >Close</button>
            </div>
            <div className="text-[10px] text-muted">
              Every audio track render-final will mix into the export, in time order. Mirrors the backend's mix logic — if a segment shows "no audio", it won't be heard.
            </div>
            {loading && <div className="text-[11px] text-muted">Loading…</div>}
            {!loading && tracks.length === 0 && (
              <div className="text-[11px] text-muted italic">No audio tracks. Generate primary or segments to see them here.</div>
            )}
            {!loading && tracks.length > 0 && (
                ...
                <tbody>
                  {tracks.map((t, i) => (
                    <tr key={i} className={`border-t border-[#e5e5e5] ${t.warn ? 'bg-[#fdf2f1]' : ''}`}>
                      ...
                      <td className="py-1 pr-2 truncate max-w-[120px]" title={t.id || '__primary__'}>{t.id || '__primary__'}</td>
                      <td className="py-1 pr-2 text-right">{fmtTime(t.startTime)}</td>
                      <td className="py-1 pr-2 text-right">{fmtTime(t.duration)}</td>
                      <td className="py-1 pr-2 text-[10px] font-sans truncate max-w-[200px]" title={t.text}>{t.text || <span className="text-muted italic">—</span>}</td>
                      <td className="py-1 pr-2 truncate max-w-[180px]" title={t.audioKey || ''}>
