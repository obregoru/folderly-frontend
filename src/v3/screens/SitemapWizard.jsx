...
// 3-tab modal for adding an image: Upload / Pexels / Source URL.
// Each tab is independent — operator picks one image per modal open
// (closes after success), then re-opens to add more.
  ...
  const save = async (photo) => {
    ...
    try {
      await api.saveLandingImageFromPexels(landingPageId, {
        ...
        alt_text: alt,
        filename: '', // server derives from photo.alt
      })
      onSuccess()
    } catch (e) {
      setErr(e?.message || String(e))
