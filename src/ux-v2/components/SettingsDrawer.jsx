...
const TENANT_SECTIONS = [
  ...
  {
    key: 'voice', icon: '🎤', label: 'Voice & AI behavior',
    desc: 'Voice settings, hook categories, pronunciation (not keys)',
    fields: [
      'Default voice + voice settings (stability, clarity, speed)',
      'Hook categories & prompt context',
      'Pronunciation overrides',
      'Humanize blog / YouTube toggles',
    ],
  },
  {
    key: 'integrations', icon: '🔑', label: 'API keys & integrations',
    desc: 'All tenant-scoped API credentials in one place',
    fields: [
      'ElevenLabs API key (voiceover TTS)',
      'AI detection provider + key (ZeroGPT, Originality.ai, etc)',
      'Anthropic / OpenAI overrides (optional — otherwise uses platform default)',
      'Custom webhook keys (Zapier / Make.com)',
      '— Social OAuth tokens live under Platforms below',
    ],
  },
  {
    key: 'platforms', icon: '📤', label: 'Platforms',
    desc: 'OAuth connections + per-platform audiences',
    fields: [
      'TikTok (OAuth + analytics import)',
      'Instagram (OAuth + analytics import)',
      'Facebook (OAuth + Story defaults)',
...
const ACCOUNT_SECTIONS = [
  ...
  },
  {
    ...
    fields: [
      'Tenant management (create / archive)',
      'User management',
      'System secrets (Supabase, Railway, Resend — from env only, not editable here)',
      'Feature flags',
    ],
  },
]
